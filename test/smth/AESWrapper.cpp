...
bool AESWrapper::isValidKeyLength(unsigned int keyLength) const
{
	if (keyLength == 16 || keyLength == 24 || keyLength == 32) //If key length is valid for AES encryption - 128, 192 or 256 bits.
		return true;
	return false;
}
